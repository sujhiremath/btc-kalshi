...
OnTickCallback = Callable[[PriceTick], Awaitable[None]] | Callable[[PriceTick], None]


class CoinbaseFeed:
    """
    Primary BTC price feed backed by Coinbase Advanced Trade websocket.
    """

    def __init__(self, ws_url: str, on_tick: OnTickCallback) -> None:
        self._ws_url = ws_url
        self._on_tick = on_tick
        self._logger = get_logger("coinbase-feed")
    ...
    async def _handle_message(self, message: str) -> None:
        """
        Parse a Coinbase Advanced Trade BTC-USD trade message into PriceTicks.
        """
        try:
            data: dict[str, Any] = json.loads(message)
        except json.JSONDecodeError:
            self._logger.critical(
                "Failed to decode Coinbase message as JSON",
        ...
        for event in events:
            ...
            for trade in trades:
                try:
                    ...
                    volume = float(
                        trade.get("size")
                        or trade.get("size_in_product_units")
                        or 0.0
                    )
        ...
        while not self._stopped and attempts <= len(backoffs):
            try:
                async with websockets.connect(self._ws_url) as ws:
                    ...
                    # Minimal subscription to BTC-USD market trades.
                    subscribe_msg = {
                        "type": "subscribe",
                        "product_ids": ["BTC-USD"],
                ...
                attempts += 1
                self._logger.critical(
                    "Coinbase feed websocket failure",
                    extra={
                        "error": str(exc),
                        "attempt": attempts,
                    },
